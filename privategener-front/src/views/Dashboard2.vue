...
export default {
  ...
  async mounted() {
    ...
    axios.get(`http://localhost:3000/donor/${this.uid}`).then((result) => {
      this.donorDiscord = result.data.discord;
      this.license = result.data.license
      axios
        .get(`http://localhost:3000/plant2/${this.donorDiscord}`)
        .then((res) => {
          console.log(res); 
          if (res.data == null) {
  ...
  methods: {
    ...
    async addPlant() {
      ...
      if (this.license == true) {
        ...
        console.log("No puedes agregar mas plantas");
      }
      this.add = false;
      this.edit = false;
    },
